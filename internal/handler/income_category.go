...
type IncomeCategoryHandler struct {
	svc *service.IncomeCategoryService
}

func NewIncomeCategoryHandler(svc *service.IncomeCategoryService) *IncomeCategoryHandler {
	return &IncomeCategoryHandler{svc: svc}
}

func (h *IncomeCategoryHandler) GetAll(c *gin.Context) {
...
func (h *IncomeCategoryHandler) Create(c *gin.Context) {
	var req model.CreateIncomeCategoryRequest
...
func (h *IncomeCategoryHandler) Update(c *gin.Context) {
	...
	var req model.UpdateIncomeCategoryRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid request body"})
		return
	}
